...
const toggleIsANewUser = () => {
  isANewUser.value = !isANewUser.value;
};
...
const onSubmit = handleSubmit(async () => {
  try {
    ...
    if (!email || !password || (isANewUser.value && !confirmPassword)) {
      toast({
  ...
  <div>
    <Tabs default-value="signup" class="w-auto sm:w-[450px]">
      <TabsList class="grid w-full grid-cols-2 rounded-lg border-none">
        <TabsTrigger value="signup" class="data-[state=active]:bg-primary data-[state=active]:text-accent" @click="toggleIsANewUser">Sign Up
        </TabsTrigger>
        <TabsTrigger value="signin" class="data-[state=active]:bg-primary data-[state=active]:text-accent" @click="toggleIsANewUser">Sign In
        </TabsTrigger>
      </TabsList>
